...
async function startBot() {
  ...
  const sentBotExpenses = new Map(); // messageId -> { id, title, amount }
    ...
    try {
      for (const r of reactions) {
        ...
        if (emoji === '🗑️' || emoji === '❌' || emoji === '✖️') {
          const targetMsgId = r.reaction?.key?.id;
          const remoteJid = r.key?.remoteJid || r.reaction?.key?.remoteJid;
          if (!remoteJid || !targetMsgId) continue;

          const allowed = await isAllowedGroup(sock, remoteJid);
          if (!allowed) continue;

          const tracked = sentBotExpenses.get(targetMsgId);
          if (tracked) {
            console.log(`🗑️ [WhatsApp Reaction] Deleting expense "${tracked.title}" (${tracked.id}) via ${emoji}`);
            ...
            if (res.ok) {
              const data = await res.json();
              sentBotExpenses.delete(targetMsgId);
              if (data.reply) {
                const sent = await sock.sendMessage(remoteJid, { text: data.reply });
      ...
      console.error('Error handling reaction in WhatsApp bot:', err.message);
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (
        ...
        text.startsWith('🎯 *Huchas') ||
        text.startsWith('🎯 *Household') ||
        text.startsWith('🏷️ *Categorías') ||
        text.startsWith('🏷️ *Active') ||
        text.startsWith('🗑️ *Gasto') ||
        text.startsWith('🗑️ *Expense') ||
        text.startsWith('🗑️ *Último') ||
        text.startsWith('✏️ *Gasto') ||
        text.startsWith('✨ *¡Deuda') ||
        text.startsWith('✨ *Debt') ||
      ...
      // Extract quoted / replied message if available
      const contextInfo =
        msg.message.extendedTextMessage?.contextInfo ||
        msg.message.imageMessage?.contextInfo ||
        msg.message.videoMessage?.contextInfo ||
        msg.message.ephemeralMessage?.message?.extendedTextMessage?.contextInfo;
      const quotedMessage = contextInfo?.quotedMessage ? extractMessageText(contextInfo.quotedMessage) : undefined;

      const senderJid = msg.key.participant || remoteJid;
      const pushName = msg.pushName || (msg.key.fromMe ? 'Tú' : '');

      console.log(`📩 [WhatsApp] Message from ${pushName || senderJid}: "${text}"${quotedMessage ? ` (quoted: "${quotedMessage.slice(0, 30)}...")` : ''}`);
        ...
        if (res.ok) {
          ...
          if (data.reply) {
            ...
            if (sent?.key?.id) {
              ...
              // If an expense was created, track its message ID for emoji reaction deletion
              if (data.type === 'expense_created' && data.expense) {
                sentBotExpenses.set(sent.key.id, data.expense);
                if (sentBotExpenses.size > 200) {
                  const oldestExpKey = sentBotExpenses.keys().next().value;
                  sentBotExpenses.delete(oldestExpKey);
                }
              }
            }
          }
